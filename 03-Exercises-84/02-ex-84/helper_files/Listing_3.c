/*Listing 3: User-level thread library header file (uthreads.h) Background Reading and Resources*/
...
#define MAX_THREAD_NUM 100 /** Maximum number of threads (including the main thread). */
...
* Calling this function with a NULL entry_point or exceeding MAX_THREAD_NUM is
an error.
*
...
*
* Terminates the thread with the specified tid and releases all resources
allocated for it.
* The thread is removed from all scheduling structures. If no thread with the
given tid exists ,
* it is considered an error. Terminating the main thread (tid == 0) will
terminate the entire process
* (after releasing allocated resources).
*
* @param tid Thread ID to terminate.
* @return 0 on success; -1 on error. (Note: if a thread terminates itself or if
 ...
 * It is an error to block a thread that does not exist or to block the main
...
* After the sleep period expires , the thread is moved to the end of the READY
queue.
...
*
* The count starts at 1 immediately after uthread_init. Every new quantum ,
regardless of cause ,
...
/**
* @brief Returns the number of quantums the thread with the specified tid has
run.
*
...
* They provide guidance on how to structure your implementation using
...
*
* This function examines the READY queue and selects the next thread for
execution.
...
*
* Uses sigsetjmp and siglongjmp to save the current thread ’s context and restore
...
*
* Registered as the handler for timer signals , this function updates global
quantum counters
...
*
* Sets up the thread ’s jump buffer for context switching by manually assigning
the stack pointer
* and program counter using sigsetjmp/siglongjmp techniques. You may need to
perform architecture -specific
* address translation (see provided reference implementation) when initializing
the context.
*
* @param tid Thread ID.
